...
const handleAddBackgroundRemovalLayer = async () => {
  ...
  const handleAddFilterAsLayer = async (filterType, params) => {
    ...
    const fd = new FormData();
    fd.append("image", originalBlob);
    // Always use color version for layers, filters handle their own grayscale logic if needed
    fd.append("grayscale", false); 
    
    // Append parameters directly to the FormData for the initial processing
    Object.entries(params).forEach(([key, val]) => fd.append(key, val));
    ...
    try {
      ...
      // We spread 'params' directly into the layer object (root level)
      // The backend parser is now fixed to read these root level properties
      const newLayer = {
        id: nextLayerId,
        ...
        width: res.data.width,
        height: res.data.height,
        ...params // SPREAD params here (e.g. level: 50, intensity: 10)
      };
          ...
          <div className="control-section">
            ...
            <div className="transform-controls">
              ...
              <div className="transform-buttons">
                {/* FIX: Changed endpoint from 'flip/horizontal' to 'flip-horizontal' */}
